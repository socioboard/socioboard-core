...
function fbimage(src) {
    ...
    if (src.indexOf('_s.png') != -1) {
        ...
        $('#hint').html('<iframe width="420" height="345" src=' + src + '> </iframe>');
    }
...
function commentFBGroup(fbid, profileid) {
    debugger;
    var message = $("#textfb_" + fbid).val();
    if (message == "") {
        //alert("Please write something!")
    ...
    $.ajax({
        type: "POST",
        url: "../Feeds/AjaxFeeds.aspx?op=fbGroupcomment&fbid=" + fbid + "&profileid=" + profileid + "&message=" + message,
        data: '',
        contentType: "application/json; charset=utf-8",
        success: function (e) {
            debugger;
