...
namespace SocioBoard.Helper
{
    public class clsFeedsAndMessages
    {
        ...
        public DataSet bindMessagesIntoDataTable(User user)
        {
            ...
            List<FacebookFeed> alstfbmsgs = fbfeedRepository.getAllReadFbFeeds(user.Id, facebookid);
            try
            {
                foreach (FacebookFeed facebookmsg in alstfbmsgs)
                {
            ...
            List<TwitterMessage> lstmsgtwt = twtmsgRepository.getAlltwtMessagesOfUser(user.Id, twitterid);
            try
            {
            ...
            List<GooglePlusActivities> lstmsggpl = objActRepository.getAllgplusOfUser(user.Id, googleplusid);
            try
            {
