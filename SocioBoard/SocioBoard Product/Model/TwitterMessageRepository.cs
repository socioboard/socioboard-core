...
namespace SocioBoard.Model
{
    public class TwitterMessageRepository:ITwitterMessageRepository
    {
        ...
        public List<TwitterMessage> getAlltwtMessagesOfUser(Guid UserId, string profileid)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        string str = "from TwitterMessage where ReadStatus = 1 and UserId = :userid and ProfileId IN(";
