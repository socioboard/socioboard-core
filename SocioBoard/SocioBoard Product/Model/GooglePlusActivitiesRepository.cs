...
namespace SocioBoard.Model
{
    public class GooglePlusActivitiesRepository : IGooglePlusActivitiesRepository
    {
        ...
        public List<GooglePlusActivities> getAllgplusOfUser(Guid UserId, string profileId)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        string str = "from GooglePlusActivities where UserId = :userid and GpUserId IN(";
