...
namespace SocioBoard.Model
{
    public class FacebookFeedRepository : IFacebookFeedRepository
    {
        ...
        public List<FacebookFeed> getAllReadFbFeeds(Guid UserId, string profileid)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        string str = "from FacebookFeed where ReadStatus = 1 and UserId = :userid and ProfileId IN(";
