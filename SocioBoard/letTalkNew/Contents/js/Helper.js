...
function createtask(id) {
    ...
    try {
        ...
        var messagedescription = document.getElementById("msgdescription_" + ss).innerHTML;
      ...
      //  var msg = document.getElementById('messagetaskable_' + ss).innerHTML;
        ...
        $.ajax
                ...
                $("#inbox_messages").html(messagedescription);
...
function savetask() {
    ...
    try {
        ...
        if (chk) {
            ...
            ({
                type: "POST",
                url: "../Message/AjaxMessage.aspx?op=savetask&description=" + msg + "&memberid=" + dat[1] + "&comment=" + comment,
                data: '',
                contentType: "application/json; charset=utf-8",
                dataType: "html",
                success: function (msg) {
                    $("#msg_task_open").bPopup().close();
                }
            });
        } else {
...
function getGooglePlusProfiles(gpid) {
  //  alert(id);
    try {
        debugger;
        var jas = '';
        $.ajax
                            ({
                                type: "GET",
                                url: "../Message/AjaxMessage.aspx?op=gpProfile&gpid=" + gpid + "",
                                data: '',
                                crossDomain: true,
                                contentType: "application/json; charset=utf-8",
                                dataType: "html",
...
function PerformClick(id,sortdataCount,status) {
    debugger;
    // alert("id :" + id);
    debugger;
    taskid = id.split('_')[1];
    var status = id.split('_')[2];
    $("#ContentPlaceHolder1_hdnstatus").val(status);
    $("#ContentPlaceHolder1_hdnTask_id").val(taskid);

    checkStatusInfo(status, taskid,sortdataCount);
}
function checkStatusInfo(status, taskid,count) {
    debugger;
    alertify.confirm('Are you sure want to change status?', function (e) {
        if (e) {
            ...
            if (status == "False")
                $("#status_" + count).html('True');
            else {
                $("#status_" + count).html('False');
            }
