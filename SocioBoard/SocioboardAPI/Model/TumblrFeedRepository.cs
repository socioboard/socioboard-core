            ...
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        List<Domain.Socioboard.Domain.TumblrFeed> lstmsg = session.Query<Domain.Socioboard.Domain.TumblrFeed>().Where(u => u.UserId == id && u.ProfileId.Equals(profileid) && u.imageurl!=null).OrderByDescending(x => x.date).Skip(Convert.ToInt32(noOfDataToSkip)).Take(6).ToList<Domain.Socioboard.Domain.TumblrFeed>();
