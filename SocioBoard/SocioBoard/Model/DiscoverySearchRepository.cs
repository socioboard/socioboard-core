...
namespace SocioBoard.Model
{
    public class DiscoverySearchRepository :IDiscoverySearchRepository
    {
        ...
        public List<DiscoverySearch> getResultsFromKeyword(string keyword,Guid id)
        {
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        List<DiscoverySearch> lst = session.CreateQuery("from DiscoverySearch where SearchKeyword = :keyword and UserId=:UserId")
                         .SetParameter("keyword", keyword)
