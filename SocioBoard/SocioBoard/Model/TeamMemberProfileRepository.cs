...
namespace SocioBoard.Model
{
    public class TeamMemberProfileRepository:ITeamMemberProfileRepository
    {
        ...
        public ArrayList getLimitProfilesOfUser(Guid TeamId, int limit)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    ...
                    NHibernate.IQuery query = session.CreateQuery("from TeamMemberProfile where TeamId = :teamid and ProfileType!='googleplus' and ProfileType!='googleanalytics' ");
