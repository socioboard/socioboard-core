...
namespace SocioBoard.Model
{
    public class ScheduledMessageRepository : IScheduledMessageRepository
    {
        ...
        public void UpdateProfileScheduleMessage(Guid Id, string profileid, string message, string network)
        {
            try
            {
                using (NHibernate.ISession session = SessionFactory.GetNewSession())
                {
                    using (NHibernate.ITransaction transaction = session.BeginTransaction())
                    {
                        try
                        {
                            NHibernate.IQuery query = session.CreateQuery("update ScheduledMessage set ProfileId =:prof,ShareMessage =:share,ProfileType  =:prot where Id = :id");
                            query.SetParameter("id", Id);
                            query.SetParameter("prof", profileid);
                            query.SetParameter("share", message);
                            query.SetParameter("prot", network);
        ...
        public void UpdateProfileScheduleMessageStatus(Guid Id, string Status)
        {
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                     //   ScheduledMessage schMsg= getScheduleMessageByMessageId(Id);
        ...
        public ScheduledMessage getScheduleMessageByMessageId(Guid Id)
        {
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        NHibernate.IQuery query = session.CreateQuery("from ScheduledMessage where Id = :id");
                        query.SetParameter("id", Id);
