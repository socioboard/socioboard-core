...
function BindMessages(id) {
    ...
    try {
        ...
        $('#accordianprofiles ul li').each(function (i) {
            debugger;
            var imgtag = $(this).children("img").attr("src");

            var attrId = $(this).children("img").attr("id");

            if (imgtag.indexOf('uncheck_click') > -1) {
                debugger;
    ...
    bindmessageajax = $.ajax
        ({
            type: "POST",
            url: "../Message/AjaxMessage.aspx?op=bindMessages&profileid[]=" + arrid,
            data: '',
            contentType: "application/json; charset=utf-8",
            dataType: "html",
            success: function (msg) {
                if (msg == "No Data record found.") {
                    //No New record found..!!
...
function replyfunction(id, profiletype, messageid, userid) {
   // alert('asdasd');
    debugger;
    try {
        $.session.set('mess_id', id + ',' + profiletype + ',' + messageid + ',' + userid);
        if (id != $.session.get('mess_id')) {
            $.session.set('mess_id', id + ',' + profiletype + ',' + messageid + ',' + userid);
        }
...
function twittercomments() {
    ...
    var commentvl = $.session.get('mess_id').split(',');
