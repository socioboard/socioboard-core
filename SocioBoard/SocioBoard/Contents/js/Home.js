...
var addanotheridcheck1 =[];
var addanotheridcheck_scheduler1 =[];
var fd = new FormData();
var message='';
var filesimage='';
...
//        }
         ...
         for(i=0;i<addanotheridcheck1.length;i++)
            {
              str1 =addanotheridcheck1[i];
            
            if(composeuseridcheck==str1)
            { 
            alert('Already added!') 
            return false;
            }
...
function composemessage_scheduler(id, network) {
    ...
    try {
      ...
      for(i=0;i<addanotheridcheck_scheduler1.length;i++)
            {
            str1 =addanotheridcheck_scheduler1[i];
            
            if(composeuseridcheck_scheduler==str1)
            { 
            alert('Already added!') 
            return false;
            }
...
function addAnotherProfileforMessage(id, network) {
  ...
  //  composeuseridcheck='';
    ...
    addanotheridcheck1.push(addanotheridcheck); 
...
function addAnotherProfileforMessage_scheduler(id, network) {
  ...
  //  composeuseridcheck='';
    return false;
      ...
      addanotheridcheck_scheduler1.push(addanotheridcheck_scheduler);
...
function SendMessage() {
    debugger;
    var interval=$( "#ddlIntervalTime option:selected" ).text();
    ...
    try {
        ...
        var filesText = document.getElementById('fileuploadText').files[0];
        debugger;
      
        var fileExtension = ['txt'];
        if(filesText !=null)
        {
        if (hasExtension('fileuploadText', fileExtension)) {
                fd.append('filesText', filesText);
         }
         else
         {
...
//        if (message != ''|| filesimage !=null) {
            ...
            ({
                type: 'POST',
                url: '../AjaxHome.aspx?op=sendmessage&message=' + encodeURIComponent(message) + '&userid[]=' + chkidforusertest+'&now='+now+'&interval='+interval,
                data: fd,
...
//    
//   
//        filesimage = document.getElementById('fileuploadImage').files[0];
//     
//        debugger;
//      
//        var fileExtension = ['jpeg', 'jpg', 'png', 'gif', 'bmp'];
...
//         if ($("#textareavaluetosendmessage").val().trim().indexOf("www")>-1){
//         var longUrl = $("#textareavaluetosendmessage").val();
//                        var shortUrl = $("#textareavaluetosendmessage");
...
//                var message1 = data.results[longUrl].shortUrl.toString();
//                MessageSend(message1, filesimage);
//            },
//            error: function (err) {
//                debugger;
...
//                    else{
//                    MessageSend(message, filesimage);
//                    }
//}
...
function MessageSend(message,filesimage){
        ...
        if (message != ''|| filesimage !=null) {
            ...
            var curdate = new Date();
            var now1 = (curdate.getMonth() + 1) + "/" + curdate.getDate() + "/" + curdate.getFullYear() + " " + curdate.getHours() + ":" + curdate.getMinutes() + ":" + curdate.getSeconds();
            debugger;

            $.ajax
            ({
                type: 'GET',
                url: "../AjaxHome.aspx?op=sendmessage&message=" + message+ "&userid[]=" + chkidforusertest+"&now="+ now1,
                data: fd,
               // data:fd+'&massagee='+encodeURIComponent(message)+'&userid[]='+chkidforusertest,
                processData: false,
                contentType: false,
...
function delProfilesFromMultiusers(id) {
 ...
 var deleteId = id.split('_');
        ...
        deleteId[1]=str3;
          ...
          for(i=0;i<addanotheridcheck1.length;i++)
            {
            str2 =addanotheridcheck1[i];
            
            if(deleteId[1]==str2)
            { 
      
             var a = addanotheridcheck1.indexOf(str2);
             if(a>-1){
                addanotheridcheck1.splice(a,1);
              }
            }
            }
             ...
             for(i=0;i<addanotheridcheck_scheduler1.length;i++)
            {
            str4 =addanotheridcheck_scheduler1[i];
            
            if(deleteId[1]==str4)
            { 
      
             var a = addanotheridcheck_scheduler1.indexOf(str4);
             if(a>-1){
                addanotheridcheck_scheduler1.splice(a,1);
              }
            }
            }
